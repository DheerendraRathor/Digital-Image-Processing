...
%     myfilteredimage = imfilter(image, myfilter, 'replicate');
    ...
    [gPaddedImage, rmin, rmax, cmin, cmax] = myPaddedImage(myfilteredimage(:, :, 3), window);
    ...
    for i = rmin:rmax
        for j=cmin:cmax
            ...
            windows_image = myGetExactWindowRGB(paddedImage, [i, j], window);
            prev_mean = cat(3, 0, 0, 0);
            for k=1:iterations
                new_mean = getMean(windows_image, current_pixel, radius);
                are_close = arePixelsCloseEnough(prev_mean, new_mean, epsilon);
                if are_close == true
                    prev_mean = new_mean;
                    break
                end
